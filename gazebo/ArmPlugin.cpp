...
#define LEARNING_RATE 0.01f//0.1f for task 1
	...
	for (unsigned int i = 0; i < contacts->contact_size(); ++i)
	{
		...
		*/
		...
		if ((strcmp(contacts->contact(i).collision1().c_str(), COLLISION_ITEM  ) == 0)
         && (strcmp(contacts->contact(i).collision2().c_str(), COLLISION_POINT) == 0)) //remove this for task 1
		{
          	// collision with object reward
...
bool ArmPlugin::updateAgent()
{
	...
	*/
	...
	float joint = ref[i] + actionJointDelta * ((action % 2 == 0) ? 1.0f : -1.0f); // TODO - Set joint position based on whether action is even or odd.
...
void ArmPlugin::OnUpdate(const common::UpdateInfo& updateInfo)
{
	...
	if( hadNewState && !newReward )
	{
		...
		const float groundContact = 0.05f; //0.0 for task 1
